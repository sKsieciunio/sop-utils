...
int main(int argc, char **argv)
{
    ...
    int ballsThrown = 0, bt = 0; // bt - lokalna kopia do sprawdzania postępu
    ...
    // deklaracja tablicy argumentów dla wątków
    ...
    double meanValue = 0.0; // wartość oczekiwana rozkładu
    for (int i = 0; i < BIN_COUNT; i++)
    {
    ...
    // dla jednorazowego wywołania system swolni poniższe zasoby
    // for (int i = 0; i < BIN_COUNT; i++) pthread_mutex_destroy(&mxBins[i]);
    // free(args);

    // The resources used by detached threads cannot be freed as we are not sure
...
void *throwing_func(void *voidArgs)
{
    ...
    while (1)
    {
        ...
        int binno = throwBall(&args->seed); // binno - numer pojemnika
